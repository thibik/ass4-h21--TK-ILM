...
# Her overskriver dere pm2. Bedre å bare filtrere før plot
# pm2 <- pm2 %>% 
...
# Da er pm2 fremdeles klar til bruk
...
# Vi trenger aar some date object
pm2 <- pm2 %>% 
  mutate(
...
pm2 %>% 
  ...
  ggplot(mapping=aes(x = aar_d, y = res_m2)) +
  scale_color_viridis(discrete = TRUE, option = "D") +
  # legger til colour og size for å skille dem
  geom_line(aes(group = knavn, colour = knavn, size = knavn)) +
...
pm2_n <- pm2 %>% 
  # Velg først variablene, så group på aar_d. Endret Trade_pc_100k til Trade_pc_100K
...
```{r kom_model}
# data = a_df ellers ingen funksjon
...
### i.
```{r ny variabel, date}
# henter ut model list column
coef_df <- mod_sum$model %>% 
  # argumentet 1 gir oss koeffisientene, 2 residualene etc
  map_df(1) %>% 
  tibble()
```
