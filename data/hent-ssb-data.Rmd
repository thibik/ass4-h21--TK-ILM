...
pm2 %>% 
  map_df(is.na) %>% 
  map_df(sum) %>% 
# as.tibble() har gått ut på dato
# nå heter den as_tibble()  
#    as.tibble()
  as_tibble()
```
...
```{r}
# Jeg vil foreslå å endre navn på første komponent i uni_p så slipper en
# tekst-strenger som går langt ut i margen. Disse navnene til SSB er ikke særlig
# R vennlige
names(uni_p)[[1]] <- "desc"
```
...
```{r}
# bruker desc for å referere til første element i uni_p listen
# slipper uendelig lange navn som går utenfor margen
uni_p <- tibble(
